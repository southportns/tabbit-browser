...
class NetworkManager {
  ...
  /** 启用网络监控（Network + Fetch 拦截）。用于 CLI 等需要拦截/日志的场景。 */
  async enable() {
  ...
  /** 仅启用 Network 域（Cookie/缓存/限速等不需要 Fetch 拦截的操作）。
   *  避免与持久化拦截器的 Fetch.enable 冲突（重复 Fetch 会导致同一请求被暂停两次）。 */
  async enableNetworkOnly() {
    await this.session.send('Network.enable');
    this.session.on('Network.requestWillBeSent', (params) => {
      this._requestLog.push({
        id: params.requestId,

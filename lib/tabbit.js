...
class TabbitBrowser {
  constructor(options = {}) {
    ...
    // 独立实例模式：使用独立端口 + 独立 user-data-dir，与用户浏览器完全隔离
    this.isolated = options.isolated || false;
    this.userDataDir = options.userDataDir || null;
    this._proc = null; // 保存子进程引用，便于关闭
  }

  /** 检查指定端口上是否有浏览器在运行且有调试端口 */
  ...
  /** 检测可用端口（从 startPort 开始递增，直到找到空闲端口） */
  ...
  async launch(options = {}) {
    ...
    // 独立实例模式：不杀掉已有进程，使用独立端口和 user-data-dir
    if (this.isolated) {
      // 自动选择空闲端口
      if (!this.port || this.port === 9222) {
        this.port = await this.findAvailablePort(9223);
      }
      // 创建独立的 user-data-dir
      if (!this.userDataDir) {
        const os = require('os');
        const path = require('path');
        this.userDataDir = path.join(os.tmpdir(), `tabbit-mcp-${this.port}`);
      }
      const fs = require('fs');
      ...
      this._proc = spawn(this.executablePath, args, { detached: true, stdio: 'ignore' });
      this._proc.unref();
      ...
      throw new Error('独立浏览器实例启动超时，调试端口未就绪');
    }
  ...
  /** 关闭独立实例（仅杀死自己启动的进程，不影响用户的浏览器） */
  async close() {
    if (this._proc) {
      try {
        process.kill(this._proc.pid);
      } catch {}
      this._proc = null;
      await sleep(1000);
      return true;
